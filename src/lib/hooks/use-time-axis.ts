...
const countDaysInRange = (rangeEnd: number, rangeStart: number) => Math.ceil((rangeEnd - rangeStart) / oneDay);

const getInnerHourMarkers = (hourIndex: number, zoom: number, factor: number, time: string, hourFormat: string) => {
	return Array.from({ length: factor - 1 }).map((_, factorIndex) => {
		...
		return {
			...
			time: moment(time, hourFormat).add(minutesDigits, 'minutes').format(hourFormat)
		};
	});
};
...
const useTimeAxis = ({ rangeStart, rangeEnd, zoom, axisOptions, hourFormat = 'H:mm' }: Props) => {
	...
	const hoursInRange = useMemo(() => Array.from({ length: 24 * daysInRange.length }), [daysInRange]);
	...
	const hoursValues = useMemo(() => {
		...
		return hoursInRange.reduce(
			(markers: { left: number; time: string }[], _, index) => {
				...
				if (minutesFactor) {
					markers.push(...getInnerHourMarkers(index, zoom, minutesFactor, roundHourMarker.time, hourFormat));
				}
