...
const wheelEventAdapter = ({ deltaX, deltaY, shiftKey }: WheelEvent) =>
	shiftKey && deltaY !== 0 ? { deltaX: deltaY, deltaY: deltaX } : { deltaX, deltaY };
...
const shouldScroll = ({ code }: KeyboardEvent) => ['ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight'].includes(code);

export function notifyNeedToScrollHorizontal({ target, scrollLeft, immediate }: ScrollEvent & { immediate?: boolean }) {
...
export const useTimelineScroll = ({
	...
	useEffect(() => {
		...
		const dispatchScroll = (deltaX: number, deltaY: number, event: Event) => {
			if (maxHorizontalScroll !== 0 && Math.abs(deltaX) >= Math.abs(deltaY)) {
				event.preventDefault();

				const targetScrollLeft = Math.min(Math.max(lastScrollLeft + deltaX, 0), maxHorizontalScroll);
			...
			if (maxVerticalScroll !== 0 && syncScrollOnWheel) {
				event.preventDefault();
				const newScrollTop = Math.min(Math.max(lastScrollTop + deltaY, 0), maxVerticalScroll);
		...
		const onScrollBar = (e: Event) => {
			...
			if (syncScrollOnWheel && onScroll && lastScrollTop !== newScrollTop) {
				lastScrollTop = newScrollTop;
		...
		const horizontalScroll = ({ scrollLeft }: ScrollEvent) => {
			if (scrollElement) scrollElement.style.transform = `translate(${-scrollLeft}px, 0)`;
			onWheelCallback?.();
		};
		...
		if (scrollElement && container) {
			resizeObserver = new ResizeObserver(entries => {
				...
				const targetScroll = Math.min(Math.max(lastScrollLeft, 0), maxHorizontalScroll);
	...
	const handleThumbMousemove = useCallback(
		(e: MouseEvent) => {
			if (isDragging) {
				...
				const newScrollLeft = ((initialScrollLeft + deltaX) / trackSize) * rangeWidth;
				const scrollLeft = Math.min(Math.max(newScrollLeft, 0), rangeWidth - viewWidth);
		...
		[trackRef, isDragging, scrollStartPosition, viewWidth, rangeWidth, initialScrollLeft]
	);
