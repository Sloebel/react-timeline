...
import { notifyNeedToScrollHorizontal, scrollOnZoom } from './use-timeline-scroll';
...
const isLessThenHour = (start: number, end: number) => end - start < hourInMili;
...
const useZoom = ({
	...
	const calcRangeWidth = (_zoom: number) => {
		let zoomToSet = _zoom;
		let rangeWidthToSet = rangeStart && rangeEnd ? ((rangeEnd - rangeStart) / hourInMili) * _zoom : 0;

		if (rangeWidthToSet < viewWidth) {
			zoomToSet = Math.floor(viewWidth / ((rangeEnd - rangeStart) / hourInMili));
	...
	useEffect(() => {
		...
		rows.length && setZoomRange(zoomProvider.calcZoomRange(scrollLeft, newZoom, viewWidth, rangeStart));
	...
	const zoomIn = () => {
		...
		setZoomRange(zoomProvider.calcZoomRange(scrollLeft, newZoom, viewWidth, rangeStart));
	};
	...
	const fitRange = (start: number, end: number) => {
		...
		const zoomToSet = Math.ceil(viewWidth / ((endRange - startRange) / hourInMili));

		const { newZoom } = calcRangeWidth(zoomToSet);

		const scrollLeft = (Math.max(startRange - rangeStart, 0) / hourInMili) * zoomToSet;

		notifyNeedToScrollHorizontal({ scrollLeft });

		newZoom && setZoomRange(zoomProvider.calcZoomRange(scrollLeft, newZoom, viewWidth, rangeStart));
	};
