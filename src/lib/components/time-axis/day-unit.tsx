...
const useTimeValueSticky = (dayRef: React.RefObject<HTMLSpanElement>, left: number, maxLeft: number) => {
	...
	useEffect(() => {
		const setTranslateX = (scrollLeft: number) => {
			if (dayRef.current) {
				...
				if (translateX > 0 && translateX < maxLeft - left - dayRef.current.offsetWidth) {
