...
const TimeLineItemTooltip = ({
	...
	const shouldRenderTooltip = targetElement && !!item && isFunction(tooltipRender);
	const items = useMemo(() => (item && 'items' in item ? (item as TimeLineClusterItem).items : [item]), [item]);
	...
	const tooltipContent = useMemo(() => {
		...
		return tooltipRender(items as TimelineItem[], {
			removeTooltip,
			redrew: () => (updateRef.current ? updateRef.current() : Promise.resolve({}))
		});
	}, [shouldRenderTooltip, tooltipRender, items, removeTooltip]);

	const container =
		(tooltipContainerSelector && document.querySelector(`.${tooltipContainerSelector}`)) || document.body;
