...
const GRID_INNER_SCROLL_CONTAINER =
...
const extraScrollRows = (height: number, rowsLength = 0) =>
...
const ReactTimeline = forwardRef<TimelinePublicApi, Props>(
  (
    ...
    const [gridInnerScrollRef, setGridInnerScrollRef] =
    ...
    useEffect(() => {
      gridContainerRef.current = document.getElementById(
    ...
    useEffect(() => {
      ...
      innerScrollElement =
        gridContainerRef.current?.querySelector(GRID_INNER_SCROLL_CONTAINER);
    ...
    const itemClick = (
      ...
      const criteriaFn =
        typeof tooltipCriteria === 'function' ? tooltipCriteria : () => true;
    ...
    function cellRangeRenderer(props: GridCellRangeProps) {
      ...
      children.unshift(
        <GridLines key="grid-lines" hoursValues={hoursValues} />
      );
      children.push(
        <CurrentTimeBar
    ...
    return (
      <>
        ...
        >
          ...
          <div
            className={classNames(styles.shadow, { [styles.scroll]: _scrollTop })}
          />
        </div>
